...
public class JPanelSquare extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (GameUtil.DataTop[y][x] == 0) {
            g.drawImage(GameUtil.NoFlagImage, 0, 0, width, height, null);
        }
        if (GameUtil.DataTop[y][x] == 1) {
            this.setOpaque(false);
        }
        if (GameUtil.DataTop[y][x] == 2) {
            g.drawImage(GameUtil.FlagImage, 0, 0, width, height, null);
        }

        // 高亮显示
        if (isHighlighted && (GameUtil.DataTop[y][x] == 0 || GameUtil.DataTop[y][x] == 2)) {
            g.setColor(new Color(255, 255, 255, 100));
            g.fillRect(3, 3, width - 8, height - 8);
        }
    }
}